...
function findGroup(groupKey) {
    ...
    el = document.getElementById(`${alt}-filter`)
      || document.querySelector(`.${alt}-filter`);
    if (el) return el;

    if (!groupKey.startsWith("filter-")) {
        el = document.getElementById(`filter-${groupKey}`)
          || document.querySelector(`.filter-${groupKey}`);
        if (el) return el;
    }
...
async function loadCardResources(card) {
    ...
    try {
        ...
        if (!result.voices.fanfare) {
            const found = result.voices.other.find(o =>
                /play|fanfare|enter|summon|sample|01/.test(o.name)
            );
...
function nextQuestion() {
    ...
    loadCardResources(currentCard)
        .then(() => {})
        .catch(err => console.warn("preload err", err));
}
...
function updateProgressUI() {
    ...
    const elRemaining = document.getElementById("remainingCount");
    if (elRemaining) elRemaining.textContent = Math.max(
        0,
